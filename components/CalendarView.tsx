  ...
  return (
    ...
    <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden select-none">
      ...
      {viewMode !== ViewMode.DAY && (
        <div className="grid grid-cols-7 bg-red-800 text-white flex-shrink-0">
          {weekDays.map((day) => (
      ...
      <div className={`grid ${gridColsClass} auto-rows-fr flex-1 bg-orange-100 gap-[1px] overflow-y-auto pb-32 lg:pb-0`}>
        {days.map((day, idx) => {
          ...
          return (
            <div
              ...
              className={`${minHeightClass} bg-white p-1 sm:p-2 flex flex-col relative transition-colors group/day cursor-pointer hover:bg-orange-50/50
                ${viewMode === ViewMode.MONTH && !isSameMonth(day, currentDate) ? 'text-gray-300 bg-gray-50' : 'text-gray-700'}
            ...
            >
              <div className="flex justify-between items-start group relative mb-1 pointer-events-none w-full">
                ...
                {viewMode === ViewMode.DAY ? (
                  ...
                  <span className={`text-[10px] leading-none mt-1 ml-0.5 ${isLunarFirst ? 'text-red-400 font-bold' : 'text-gray-300'}`}>
                    {lunarDisplay}
                  </span>
                )}
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                         <div className="truncate flex-1 pr-1 flex items-center gap-2">
                           ...
                           <span className={`font-bold opacity-80 whitespace-nowrap ${viewMode === ViewMode.DAY ? 'text-base' : ''}`}>{task.time}</span>
                      ...
                      {(viewMode === ViewMode.DAY || viewMode === ViewMode.WEEK) && task.description && (
                        <div className="mt-1 text-xs opacity-70 line-clamp-2 pl-1">
                          {task.description}
                        </div>
                      )}
                      ...
                      {/* Hover Menu */}
                      <div className="hidden group-hover/task:flex items-center bg-white/95 rounded shadow-sm absolute right-1 top-1 z-20 border border-gray-100">
                  ...
                  {dayTasks.slice(0, 6).map(task => {
                    ...
                    return (
                      <div 
                          ...
                          className={`w-1.5 h-1.5 rounded-full ${task.completed ? 'bg-gray-300' : (tagConfig?.dot || 'bg-orange-500')}`}
                      />
                    )
                  })}
