...
interface AiAssistantProps {
  tasks: Task[];
  externalOpen?: boolean; // Nav button tương thích mobile
  onClose?: () => void;
}
...
const AiAssistant: React.FC<AiAssistantProps> = ({ tasks, externalOpen, onClose }) => {
  ...
  return (
    <>
      {/* Floating Button - ấn trên mobile (dùng bottom nav thay thế) */}
      <button
        onClick={() => setIsOpen(true)}
        className={`hidden lg:flex fixed z-40 bottom-8 right-8 w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 text-white rounded-full shadow-xl items-center justify-center transition-transform hover:scale-110 active:scale-95 border-2 border-white dark:border-gray-800 ${isOpen ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`}
      >
        <Bot size={28} />
      </button>
